...
async void Run(string workspace, string name) {
    ...
    UndertaleSprite GetSprite(string name) {
        lock(Data.Sprites) {
            ...
            if (baseSprite == null) {
                log.WriteLine($"[CloneSprite]missing sprite: {name}");
                return null;
            }
            result = new () {
        ...
        Parallel.ForEach(iter, file => {
            ...
            Parallel.ForEach(lines.Skip(1), line => {        
                ...
                if (sprite.CollisionMasks.Count > 0) {
                    log.WriteLine($"[ImportSprites] size changed for sprite {name} with CollisionMask");
                }
                sprite.Width = (uint)w;
    ...
    async Task MainTask() {
        ...
        ReplaceFonts();
        WriteRestoredJson(Path.Combine(resultPath, "lang_en.json"),await cn, await en, await fmt);
    }
