...
function scr_rhythmgame_draw_border(arg0, arg1 = false)
{
    ...
    if (invc > 0)
    {
        _bordercolor = merge_color(note_color[1], 255, scr_rhythmgame_damage_flash());
    }
    draw_set_color(merge_color(0, _bordercolor, brightness));
    if (!arg1 && !demo_mode)
    {
        ...
        draw_sprite_ext(_icon, _hurt ? 5 : 0, arg0 - _hoff - 20, bottomy - 234, 1, 1, 0, merge_color(0, 16777215, brightness), 1);
...
function scr_rhythmgame_draw_combo(arg0)
{
    if (combo > 0)
    {
        ...
        draw_text_transformed_color(arg0 + 2, bottomy - 121, _combotext, (combo >= 100) ? 3.5 : 5, 5, 0, 6710886, 6710886, 12303291, 12303291, 0.25);
        draw_text_transformed_color(arg0 + 2, bottomy - 60, stringsetloc("COMBO", "scr_rhythmgame_draw_slash_scr_rhythmgame_draw_gml_127_0_b"), 2, 2, 0, 12303291, 12303291, 14540253, 14540253, 0.25);
        draw_set_halign(0);
    }
}
...
function scr_rhythmgame_draw_chart(arg0, arg1, arg2 = false)
{
    ...
    _notecol[0] = merge_color(note_color[0], 255, _flash);
    _notecol[0] = merge_color(0, _notecol[0], brightness);
    _notecol[1] = merge_color(note_color[1], 255, _flash);
    _notecol[1] = merge_color(0, _notecol[1], brightness);
    _notecol[2] = merge_color(note_color[2], 255, _flash);
    _notecol[2] = merge_color(0, _notecol[2], brightness);
    var _yellow = merge_color(0, merge_color(65535, 255, _flash), brightness);
    var _gold = merge_color(0, 7532031, brightness);
    var _orange = merge_color(0, merge_color(4235519, 255, _flash), brightness);
    var _white = merge_color(0, merge_color(16777215, 255, _flash), brightness);
    var _gray = merge_color(0, merge_color(8421504, 255, _flash), brightness);
    if (instrument == 1 && !arg2)
    {
        draw_sprite_ext(6115, 4, arg1 - 20, bottomy, 1, 1, 0, _notecol[0], 1);
        draw_sprite_ext(6115, 4, arg1 + 20, bottomy, 1, 1, 0, _notecol[1], 1);
    }
    if (instrument == 2)
    {
        draw_set_color(0);
    ...
    if (instrument == 2 && !arg2 && obj_rhythmgame.song_id != 10 && !clapping)
    {
        ...
        for (var i = 0; i < 3; i++)
        {
            ...
            {
                draw_sprite_ext(1535, 1, (arg1 - 30) + (30 * i), bottomy, _beam * 0.75, 1, 0, _gold, 1);
                draw_sprite_ext(1535, 2, (arg1 - 30) + (30 * i), bottomy, _beam * 0.75, 1, 0, _white, 1);
        ...
        if (cooldown > 0)
        {
            ...
            draw_sprite_ext(4784, 0, (arg1 - 30) + (30 * last_note) + notex, bottomy + 3, 1, 1, 0, _arrow, clamp01(cooldown));
        ...
        for (var i = 0; i < 2; i++)
        {
            if (note_hit_timer[i] > 0)
            {
                ...
                if (instrument == 1)
                {
                    ...
                    draw_sprite_ext(6115, 3, _cenx, bottomy, 1, 1, 0, _white, note_hit_timer[i] / 5);
                }
                if (instrument == 2)
                {
                    _cool = true;
                ...
                if (!arg2 && obj_rhythmgame.fame >= 12000 && obj_rhythmgame.combo >= 15 && _score > 0)
                {
                    if (instrument != 2)
                    {
                        draw_sprite_ext(848, (1 - _beam) * 2.9, _cenx + (_side * (1 - _ease) * 2), bottomy, 2 - _beam, 2 - _beam, 0, merge_color(_white, _col, 1 - _ease), _ease);
                    }
                    else
                    {
                        var _beam_scale = 1 - _beam;
                        draw_sprite_ext(5933, (1 - _beam) * 2.9, _cenx, bottomy, (_beam_scale * 0.5) + 1, (_beam_scale * 0.5) + 1, 0, merge_color(_white, _col, 1 - _ease), _ease);
                    }
                }
                if (instrument == 2)
                {
                    var _beam_scale = 1 - _beam;
                    draw_sprite_ext(1535, 1, _cenx, bottomy, 2, _beam_scale + 0.25, 0, _gold, clamp01(_beam * 2));
                    draw_sprite_ext(1535, 2, _cenx, bottomy, 2, _beam_scale + 0.25, 0, _white, clamp01(_beam * 2));
                }
                else if (_cool)
                {
                    draw_sprite_ext(1535, 1, _cenx, bottomy, _beam, 1, 0, _gold, 1);
                    draw_sprite_ext(1535, 2, _cenx, bottomy, _beam, 1, 0, _white, 1);
                }
                else
                {
                    draw_sprite_ext(1535, 0, _cenx, bottomy, _beam, 1, 0, _white, 1);
                }
                note_hit_timer[i] -= _hitspeed;
            }
        }
        ...
        if (notey >= _topy && (notei >= minnote || _notescore <= 0 || (_loopcheck && _notealive)))
        {
            ...
            if (_notealive == 1 || _notescore <= 0)
            {
                ...
                if (arg2 && paused && do_refresh)
                {
                    ...
                    var _node = instance_create((_xleft - _xwidth) + (notetype[notei] * _xwidth) + 15, notey, 46);
                    _node.depth = depth + 1;
...
function scr_rhythmgame_draw_note(arg0, arg1, arg2, arg3 = 0)
{
    if (instrument == 2)
    {
        ...
        {
            draw_sprite_ext(2184, 0, arg0, arg1, 1, 1, 0, draw_get_color(), 1);
        }
    }
    ...
    else
    {
        ...
        draw_sprite_ext(1234, 0, arg0, arg1, 1, 1, 0, draw_get_color(), 1);
    }
}
...
function scr_rhythmgame_draw_note_long(arg0, arg1, arg2, arg3, arg4 = false)
{
    if (instrument == 2)
    {
        ...
        draw_sprite_ext(2184, 0, arg0, arg1 - arg3, 1, 1, 0, _col, 1);
        draw_sprite_ext(2184, 0, arg0, arg1, 1, 1, 0, _col, 1);
    }
    else
    {
        arg0 = (arg0 - 20) + (arg2 * 40);
    ...
    if (arg4)
    {
        var _endColor = merge_color(65535, 4235519, clamp01(inverselerp(arg1, arg1 - 40, arg1 - arg3)));
        _endColor = merge_color(0, _endColor, brightness);
        var _startColor = merge_color(0, 65535, brightness);
        if (instrument == 2)
        {
            draw_set_color(_endColor);
            draw_sprite_ext(2184, 0, arg0, arg1 - arg3, 1, 1, 0, _endColor, 1);
        }
...
{
    draw_sprite_ext(4933, 0, arg0, arg1, 1, 1, 0, draw_get_color(), 1);
}
...
function scr_rhythmgame_songlist()
{
    ...
    if (global.chapter >= 5)
    {
        _trackname[5] = stringsetloc("Third Sanctuary", "scr_rhythmgame_draw_slash_scr_rhythmgame_draw_gml_588_0");
        _trackname[6] = stringsetloc("A DARK ZONE", "scr_rhythmgame_draw_slash_scr_rhythmgame_draw_gml_589_0");
        _trackname[7] = stringsetloc("Flower Man", "scr_rhythmgame_draw_slash_scr_rhythmgame_draw_gml_590_0");
        _trackname[8] = stringsetloc("reserved", "scr_rhythmgame_draw_slash_scr_rhythmgame_draw_gml_592_0");
        _trackname[9] = stringsetloc("4rd Sanctuary", "scr_rhythmgame_draw_slash_scr_rhythmgame_draw_gml_593_0");
    }
    return _trackname;
}
