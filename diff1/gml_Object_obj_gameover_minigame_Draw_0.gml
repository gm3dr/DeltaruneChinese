...
if (drawblackbg)
{
    draw_sprite_ext(1022, 0, 0, 0, 640, 480, 0, 0, drawblackbg);
}
if (init == 0)
{
...
if (con == 0)
{
    ...
    {
        tenna = instance_create(camerax() + 320, cameray() + 480 + 280, 696);
        tenna.depth = depth - 10;
        tenna.auto_depth = 0;
        tenna.dropshadow = true;
        tenna.preset = 2;
    }
    if ((timer == 30 && !i_ex(197)) || (timer == 90 && i_ex(197)))
    {
        ...
        {
            var band_fail = instance_create(0, 0, 932);
...
if (con == 1 && replayversion && !i_ex(62) && quitcon == 0)
{
    mus_volume(global.currentsong[1], 0, 30);
    scr_fadeout(30);
    with (1170)
...
if (sprite_exists(global.screenshot))
{
    ...
    var is_rhythm_replaywin = room == 262 && replaywin;
    ...
    if (!quitversion && !replaywin)
    {
        ...
        draw_set_alpha(1);
        staticindex += 0.25;
        draw_sprite_tiled_ext(5176, staticindex - 0.5, 0, 0, 2, 2, 16777215, staticalpha);
        draw_sprite_tiled_ext(5176, staticindex, 0, 0, 2, 2, 16777215, staticalpha);
    ...
    if (countdown)
    {
        ...
        if ((countdown_timer % 30) == 0)
        {
            ...
            {
                snd_play_pitch(342, 1 + (orchhit_pitch / 16));
                orchhit_pitch++;
            }
        }
        draw_set_halign(1);
